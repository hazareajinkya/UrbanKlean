...
// Monochrome dashboard + Urban Company accent (black / dark purple)
const PRI = "#1a1a1a";
const PRI2 = "#555555";
const PRI3 = "#333333";
const ACCENT_PURPLE = "#4c1d95";
const ACCENT_PURPLE_SOFT = "#a78bfa";
const GREEN = ACCENT_PURPLE;
const RED = "#9ca3af";
const SLATE = "#6b7280";
const BLUE = "#374151";
const PINK = "#6b7280";
...
const LEAD_FUNNEL = [
  ...
  { stage: "Paid", count: 928, color: "#6d28d9" },
];
      ...
      <main className="max-w-7xl mx-auto p-6 lg:p-10 space-y-6">
          ...
          <ChartCard title="7-Day Tag Trends" subtitle="How conversation tags are evolving">
            <ResponsiveContainer width="100%" height={280}>
              <LineChart data={TAG_TRENDS} margin={{ top: 10, right: 10, left: -10, bottom: 0 }}>
                ...
                <Line type="monotone" dataKey="booking_completed" stroke={ACCENT_PURPLE} strokeWidth={2.5} dot={{ r: 3 }} activeDot={{ r: 5 }} />
        ...
        <div className="grid lg:grid-cols-3 gap-6">
          <ChartCard title="Sentiment Analysis" subtitle="Detected from voice tone & content">
            ...
            <div className="space-y-2">
              <SentimentRow icon={Smile} label="Positive" pct={positivePct} count={SENTIMENT[0].value} color={ACCENT_PURPLE} bg="bg-[#4c1d95]/20" />
                ...
                return (
                  <div key={stage.stage}>
                    ...
                    <div className="h-8 bg-white/[0.06] rounded-md overflow-hidden">
                      <div className="h-full rounded-md transition-all duration-700 flex items-center justify-end pr-3 text-xs font-medium text-white"
                        style={{ width: `${pct}%`, backgroundColor: stage.color === PRI ? ACCENT_PURPLE : stage.color }}>
                        {Math.round(pct)}%
                      </div>
                    </div>
                  </div>
                ...
                <BarChart data={PEAK_HOURS} margin={{ top: 10, right: 10, left: -10, bottom: 0 }}>
                  ...
                  <Bar dataKey="calls" radius={[6, 6, 0, 0]}>
                    {PEAK_HOURS.map((entry) => (
                      <Cell key={entry.hour} fill={entry.calls > 120 ? ACCENT_PURPLE : entry.calls > 60 ? ACCENT_PURPLE_SOFT : "rgba(255,255,255,0.2)"} />
                    ))}
                  </Bar>
                </BarChart>
              </ResponsiveContainer>
              <div className="flex items-center gap-4 text-xs text-white/30 mt-2 pt-3 border-t border-white/[0.08]">
                <div className="flex items-center gap-1.5"><span className="size-2.5 rounded-sm bg-[#4c1d95]" />Peak (120+)</div>
                <div className="flex items-center gap-1.5"><span className="size-2.5 rounded-sm bg-[#a78bfa]" />Busy (60+)</div>
