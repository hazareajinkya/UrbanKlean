...
export async function POST(req: NextRequest) {
  try {
    ...
    const { wid, orderId, orderNumber, note, fullRefund } =
      refundOrderSchema.parse(body);
    ...
    const result = await shopifyService.refundOrder({
      shop: integration.id,
      orderId,
      orderNumber,
      note,
    ...
    return errorResponse(error.message || "Failed to refund order", 500);
  }
}
